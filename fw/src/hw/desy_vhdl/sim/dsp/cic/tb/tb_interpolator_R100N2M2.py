'''
    Test of the CIC filter interpolator with
    R=100
    N=2
    M=2
'''
...
@cocotb.test()
async def test_interpolation(dut):
    """
        Test interpolation
    """
    ...
    trace_y_cic = np.zeros(TRACE_LEN)
    trace_y_cic_idx = 0
    ...
    dut._log.info("Trace input length " + str(len(trace_y_decim)))

    for sample_y in trace_y_decim:
        for _ in range(np.random.randint(1, RANDOM_JITTER) + R*2):
            await FallingEdge(dut.pi_clk)
            dut.pi_stb.value = 0
    ...
    error_trace = (trace_y_cic[ERROR_SKIP_FIRST:trace_y_cic_idx] -
                   trace_y_orig[ERROR_SKIP_FIRST:trace_y_cic_idx])
    ...
    # plt.step(trace_x, trace_y_orig, label="Original signal")
    # plt.step(trace_x_decim, trace_y_decim, label="Decimated input signal")
    # plt.step(trace_x[:trace_y_cic_idx],
    #          trace_y_cic[:trace_y_cic_idx], label="Interpolated CIC output")
